 ...
 * The model being used comes from this analysis: http://ctms.engin.umich.edu/CTMS/index.php?example=MotorPosition&section=SystemModeling
 * which defines the state as: x = [angle angular_velocity current]^T . It's assumed that the entire state is measured directly so state estimation is left disabled
 */

// Start by defining a state space model, This particular model describes a DC motor but you can define any model by just declaring a Model object and then
...
// Next define a state space controller. The motor position model uses 3 states and 1 input so we'll need to specify these inside the <> brackets when declaring the controller
StateSpaceController<3, 1> controller(model);

// Lastly, since the controller isn't actually connected to a motor, we'll need to simulate one to show how the controller works. The Simulation class handles this by accepting
...
void loop()
{
  // Firstly generate some measurements from the simulator. Since this model assumes that the entire state can be observed y is the same size as x
  // and contains measurements for motor angle, velocity and current. If we were controlling an actual motor, these observations would measured from
  // physical sensors attached to the motor.
  ...
  // would be used to set the motor voltage.
